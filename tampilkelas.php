...
<body>
    ...
    <table border="1" cellpadding="8" cellspacing="0" class="table">
        ...
        <tr>
            <td class="td"><?= $no++ ?></td>
            <td class="td"><?= $row['nama_kelas'] ?></td>
            <td><?= $row['jurusan'] ?></td>
            <td>
                <a href="?hal=editkelas&id=<?= $row['id'] ?>" onclick="return confirm('YAKIN AKAN DIEDIT?')" class="btn-edit"></a> &nbsp;
                <a href="?hal=tampilkelas&id=<?= $row['id'] ?>" onclick="return confirm('YAKIN AKAN DIHAPUS?')" class="btn-delete"></a>
            </td>
        </tr>
        <?php endforeach; ?>
    </table>
