...
<table border="1" cellpadding="8" cellspacing="0" class="table">
    ...
    <tr>
        ...
        <td><?= $row['username'] ?></td>
        <td>
            <a href="?hal=edituser&id=<?= $row['iduser'] ?>" onclick="return confirm('YAKIN AKAN DIEDIT?')" class="btn-edit"></a> &nbsp;
            <a href="?hal=tampiluser&id=<?= $row['iduser'] ?>" onclick="return confirm('YAKIN AKAN DIHAPUS?')" class="btn-delete"></a>
        </td>
    </tr>
    <?php endforeach; ?>
</table>
